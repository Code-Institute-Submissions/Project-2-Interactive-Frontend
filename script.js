...
$(function () {
  ...
  // for Map loading
  let singapore = [1.37695, 103.806926];
  ...
  function startGame() {
    ...
    axios.get(url + pokemonNumber).then(function (response) {
      ...
      if (response.data.types[0].type.name == "normal") {
        ...
        wantedPokemonMarker.bindPopup(
          `<p>Captured!<p><p>${response.data.name.toUpperCase()}</p>`
        );
    ...
    for (let r = 0; r < 30; r++) {
      ...
      axios.get(url + pokemonNumber).then(function (response) {
        ...
        if (response.data.types[0].type.name == "normal") {
          ...
          randomPokemonMarker.bindPopup(
            `<p>${response.data.name.toUpperCase()}</p>`
          );
      ...
      xAxes: [
        {
          display: true,
          scaleLabel: {
            display: true, labelString: "Rounds",
            fontColor: "black", fontSize: 20, fontStyle: "bold"
          },
          ticks: { fontColor: "black", fontSize: 14 },
      ...
      yAxes: [
        {
          display: true,
          scaleLabel: {
            display: true, labelString: "Clicks",
            fontColor: "black", fontSize: 20, fontStyle: "bold"
          },
          ticks: { beginAtZero: true, fontColor: "black", fontSize: 14,},
          gridLines: { color: "black" },
        },
      ],
    },
